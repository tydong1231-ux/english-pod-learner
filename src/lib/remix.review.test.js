...
vi.mock('./openaiVocab', () => ({ OpenAIVocabService: class {
    async createChatCompletion({ messages }) {
        mock.prompts.push(messages.at(-1).content);
        const response = mock.queue.length ? mock.queue.shift() : mock.response;
...
import { RemixOpenAI } from './remixOpenAI';
...
it('accepts a valid exercise and saves one source-scoped record', async () => {
    const item = await RemixService.generate({ sourcePodcastId: 'podcast', sourceSegmentIndex: 0, segment: { text: sentence }, openaiApiKey: 'fake' });
    expect(item.phrase).toBe(phrase);
    expect(mock.writes).toHaveLength(1);
});
it('does not save noAlternative responses', async () => {
    mock.response = { noAlternative: true };
    await RemixService.generate({ sourcePodcastId: 'podcast', sourceSegmentIndex: 0, segment: { text: sentence }, openaiApiKey: 'fake' });
    expect(mock.writes).toHaveLength(0);
});
it('REQUIREMENT: rejects a previously shown phrase returned by the provider', async () => {
    await expect(new RemixOpenAI({ apiKey: 'fake' }).generateExercise({ sourceSentence: sentence, excludedPhrases: [phrase] })).rejects.toThrow();
});
it('REQUIREMENT: excludes the current target from previous-phrase candidates', async () => {
    mock.previous = [phrase];
    await RemixService.generate({ sourcePodcastId: 'new-podcast', sourceSegmentIndex: 0, segment: { text: sentence }, openaiApiKey: 'fake' });
...
it('REQUIREMENT: rejects a phrase which is only a substring of a source word', async () => {
    ...
    await expect(new RemixOpenAI({ apiKey: 'fake' }).generateExercise({ sourceSentence: 'The accounting software is reliable.' })).rejects.toThrow();
});

const generate = excludedPhrases => RemixService.generate({ sourcePodcastId: 'podcast', sourceSegmentIndex: 0, segment: { text: sentence }, openaiApiKey: 'fake', excludedPhrases });
